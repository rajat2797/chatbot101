...
def youtube_search(fbid,message_text):
	...
	response_msg = json.dumps({"recipient":{"id":fbid}, "message":{"attachment":{"type":"template","payload":{"template_type":"button","text":message_text.capitalize().replace('+',' '),"buttons":[{"type":"web_url","url":output_text,"title":"Show Website"}]}}}})
	status = requests.post(post_message_url, headers={"Content-Type": "application/json"},data=response_msg)
